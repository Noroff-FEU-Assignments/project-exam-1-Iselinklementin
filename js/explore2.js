...
function createArticles(urlResponse, tagResponse, media) {
    ...
    for(let i = 0; i < urlResponse.length; i++) {
        ...
        headPost.innerHTML += `<a href="detail.html?id=${result.id}">
            ...
            if ( i < 2) {
                const twoPosts = document.querySelector(".wrap-two-posts");
                console.log("this")
                twoPosts.innerHTML += `<a href="detail.html?id="${urlResponse[0].id}">
                                        <article class="blog-one-top blog-two-top">
                                        <figure>
